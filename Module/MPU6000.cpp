...
//	EEPROM *rom = EEPROM::get_instance();
	...
	dev->write_register( MPU6000_PWR_MGMT_1, 0x80);// Reset Device
	hal.scheduler->delay(10);
	
	dev->write_register( MPU6000_PWR_MGMT_1, 0x00);
	dev->write_register( MPU6000_PWR_MGMT_2, 0x00);	// Enable Acc & Gyro
	dev->write_register( MPU6000_USER_CTRL, 0x10);	// disable I2c
	dev->write_register( MPU6000_CONFIG, 0x00);		// no filter
	...
	do
	{
		dev->write_register( MPU6000_GYRO_CONFIG, 0x08);
		hal.scheduler->delay(10);
		readGyroRange();
	}while( gyroRange != 500 );
	...
	do
	{
		dev->write_register( MPU6000_ACCEL_CONFIG, 0x10);
		hal.scheduler->delay(10);
		readAccRange();
...
//	float accel_offset_r[3] = {0.0378675461,0.00935792923,0.040371418};
//	float accel_T_r[3][3] = {{0.998772323,0.0170307066,-0.000855025079},
//													 {-0.0125333043,0.999121785,0.0158568211},
//	                         {0.00146463292,-0.0149782943,1.00010467}};
	
	float accel_offset_r[3] = {0.0373477936,0.00899457932,0.0444927216};
	float accel_T_r[3][3] = {{0.997918189,0.0434156284,-0.00388677209},
													 {-0.0417765975,0.998679698,-0.00137836346},
	                         {0.00374708953,0.00230528531,0.999422967}};
...
uint8_t MPU6000::readRaw( void )
{
	...
	dev->read_registers( reg, temp, 14 );
...
uint8_t MPU6000::selfTest()
{
	...
	dev->write_register( MPU6000_GYRO_CONFIG, BITS_FS_250DPS);
	dev->write_register( MPU6000_ACCEL_CONFIG, 0x10);  // +-8g
	...
	dev->write_register( MPU6000_GYRO_CONFIG, BITS_FS_250DPS | 0xE0); 
	dev->write_register( MPU6000_GYRO_CONFIG, 0x10 | 0xE0); 
...
//		MPU6000_ReadGyroRaw();
	...
	dev->read_registers(0x0D, trim, 4);
...
{
	readRaw();
	accelData = accelRawData*MPU6000_8g;
//	accelData = accelT * (accelRawData*MPU6000_8g - accelOffset);
	tempData = tempRawData/340 + 36.53f;
	gyroData = (gyroRawData-gyroOffset)*MPU6000_500rps;
...
//	uint64_t t_start_us = hrt_absolute_time_us();
...
//			
//			t_us = hrt_absolute_time_us();   // record now time
...
//		delayMs(1000);
//		ledPD1.LED_off();
//	  return 0;        // correspond [ g, 0, 0 ] //x up
//	}
