...
//    void send_status_report(mavlink_channel_t chan);
    ...
    void set_baro_alt_noise(float noise) { _baroAltNoise = noise; };
...
private:
    ...
    int8_t  _enable;               // zero to disable EKF3
    ...
    int8_t  _fusionModeGPS;        // 0 = use 3D velocity, 1 = use 2D velocity, 2 = use no velocity
    ...
    int8_t _altSource;             // Primary alt source during optical flow navigation. 0 = use Baro, 1 = use range finder.
    float _rngNoise;             // Range finder noise : m
    int8_t _gpsCheck;              // Bitmask controlling which preflight GPS checks are bypassed
    int8_t _imuMask;               // Bitmask of IMUs to instantiate EKF3 for
    ...
    int8_t _logging_mask;          // mask of IMUs to log
    ...
    float _rngBcnNoise;          // Range beacon measurement noise (m)
    int16_t _rngBcnInnovGate;      // Percentage number of standard deviations applied to range beacon innovation consistency check
    int8_t  _rngBcnDelay_ms;       // effective average delay of range beacon measurements rel to IMU (msec)
    float _useRngSwSpd;          // Maximum horizontal ground speed to use range finder as the primary height source (m/s)
    float _accBiasLim;           // Accelerometer bias limit (m/s/s)
    ...
    float _visOdmVelErrMax;      // Observation 1-STD velocity error assumed for visual odometry sensor at lowest reported quality (m/s)
    float _visOdmVelErrMin;      // Observation 1-STD velocity error assumed for visual odometry sensor at highest reported quality (m/s)
    float _wencOdmVelErr;        // Observation 1-STD velocity error assumed for wheel odometry sensor (m/s)
