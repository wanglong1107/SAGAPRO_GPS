                                       ...
                                       enum MS56XX_TYPE ms56xx_type)
{
    ...
    AP_Baro_MS56XX *sensor = new AP_Baro_MS56XX(baro, static_cast<AP_HAL::Device *>(dev), ms56xx_type);
...
//    }
    ...
    _dev->write_register(CMD_MS56XX_RESET, 1);
...
//    printf("%s found on bus %u address 0x%02x\n", name, _dev->bus_num(), _dev->get_bus_address());
    ...
    _dev->write_register(ADDR_CMD_CONVERT_TEMPERATURE, 1);
    _state = 0;
...
uint16_t AP_Baro_MS56XX::_read_prom_word(uint8_t word)
{
    ...
    uint8_t val[2];
    if (!_dev->read_registers(reg, val, sizeof(val))) {
        return 0;
    }
...
{
    uint8_t val[3];
    if (!_dev->read_registers( CMD_MS56XX_READ_ADC, val, sizeof(val))) {
        return 0;
    }
...
void AP_Baro_MS56XX::_timer(void)
{
    ...
    if (!_dev->write_register( next_cmd, 1)) {
        return;
    }
