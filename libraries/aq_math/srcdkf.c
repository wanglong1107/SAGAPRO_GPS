...
srcdkf_t *srcdkfInit(int s, int m, int v, int n, SRCDKFTimeUpdate_t *timeUpdate)
{
	...
	f = (srcdkf_t *)calloc(1, sizeof(srcdkf_t));
	if(NULL == f)
	{
		return NULL;
	}
	...
	ret = matrixInit(&f->Sx, s, s);
	if(ret < 0)
	{
		return NULL;
	}
	ret = matrixInit(&f->SxT, s, s);
	if(ret < 0)
	{
		return NULL;
	}
	ret = matrixInit(&f->Sv, v, v);
	if(ret < 0)
	{
		return NULL;
	}
	ret = matrixInit(&f->Sn, n, n);
	if(ret < 0)
	{
		return NULL;
	}
	ret = matrixInit(&f->x, s, 1);
	if(ret < 0)
	{
		return NULL;
	}
	ret = matrixInit(&f->Xa, s+maxN, 1+(s+maxN)*2);
	if(ret < 0)
	{
		return NULL;
	}

	ret = matrixInit(&f->qrTempS, s, (s+v)*2);
	if(ret < 0)
	{
		return NULL;
	}
	ret = matrixInit(&f->y, m, 1);
	if(ret < 0)
	{
		return NULL;
	}
	ret = matrixInit(&f->Y, m, 1+(s+n)*2);
	if(ret < 0)
	{
		return NULL;
	}
	ret = matrixInit(&f->qrTempM, m, (s+n)*2);
	if(ret < 0)
	{
		return NULL;
	}
	ret = matrixInit(&f->Sy, m, m);
	if(ret < 0)
	{
		return NULL;
	}
	ret = matrixInit(&f->SyT, m, m);
	if(ret < 0)
	{
		return NULL;
	}
	ret = matrixInit(&f->SyC, m, m);
	if(ret < 0)
	{
		return NULL;
	}
	ret = matrixInit(&f->Pxy, s, m);
	if(ret < 0)
	{
		return NULL;
	}
	ret = matrixInit(&f->C1, m, s);
	if(ret < 0)
	{
		return NULL;
	}
	ret = matrixInit(&f->C1T, s, m);
	if(ret < 0)
	{
		return NULL;
	}
	ret = matrixInit(&f->C2, m, n);
	if(ret < 0)
	{
		return NULL;
	}
	ret = matrixInit(&f->D, m, s+n);
	if(ret < 0)
	{
		return NULL;
	}
	ret = matrixInit(&f->K, s, m);
	if(ret < 0)
	{
		return NULL;
	}
	ret = matrixInit(&f->inov, m, 1);
	if(ret < 0)
	{
		return NULL;
	}
	ret = matrixInit(&f->xUpdate, s, 1);
	if(ret < 0)
	{
		return NULL;
	}
	ret = matrixInit(&f->qrFinal, s, 2*s + 2*n);
	if(ret < 0)
	{
		return NULL;
	}
	ret = matrixInit(&f->Q, s, s+n);	// scratch
	if(ret < 0)
	{
		return NULL;
	}
	ret = matrixInit(&f->R, n, n);	// scratch
	if(ret < 0)
	{
		return NULL;
	}
	ret = matrixInit(&f->AQ, s, n);	// scratch
	if(ret < 0)
	{
		return NULL;
	}

	f->xOut = (float32_t *)calloc(s, sizeof(float32_t));
	if(NULL == f->xOut)
	{
		return NULL;
	}
	f->xNoise = (float32_t *)calloc(maxN, sizeof(float32_t));
	if(NULL == f->xNoise)
	{
		return NULL;
	}
	f->xIn = (float32_t *)calloc(s, sizeof(float32_t));
	if(NULL == f->xIn)
	{
		return NULL;
	}
