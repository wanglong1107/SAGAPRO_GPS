...
int matrixInit(arm_matrix_instance_f32 *m, int rows, int cols) {
    float32_t *d;
...
int cholF(float32_t *U) {
    ...
    float32_t t0 = 1.0f / sqrtf(a11);
    float32_t t1 = a12 * t0;
    float32_t t2 = a13 * t0;
    float32_t t3 = t2 * t1;
    float32_t t4 = sqrtf(a22 - t1*t1);
            ...
            for (k=j+1; k<EstColRank; k++) {
                ...
                if (q >= r) {
                    if (q<=e2*S2[0] || fabsf(p)<=tol*q)
                        ...
                        p /= q;
                        r = 1.0-r/q;
                        vt = sqrtf(4.0*p*p+r*r);
                        c0 = sqrtf(0.5*(1.0+r/vt));
                        s0 = p/(vt*c0);
                        for (i=0; i<2*n; i++) {
                            d1 = A[i*n + j];
                            d2 = A[i*n + k];
                    ...
                    p /= r;
                    q = q/r-1.0;
                    vt = sqrtf(4.0*p*p+q*q);
                    s0 = sqrtf(0.5*(1.0-q/vt));
                    if (p<0.0) s0 = -s0;
                    c0 = p/(vt*s0);
                    for (i=0; i<2*n; i++) {
                        d1 = A[i*n + j];
