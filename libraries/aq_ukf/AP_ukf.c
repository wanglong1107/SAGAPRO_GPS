...
float VELL[3] = {0.0f,0.0f,0.0f}; 
float ACCP[3] = {0.0f,0.0f,0.0f}; 
...
float acc_nois_X = 0.0f;
float acc_nois_Y = 0.0f;
float acc_nois_Z = 0.0f;

float gyro_nois_X = 0.0f;
float gyro_nois_Y = 0.0f;
float gyro_nois_Z = 0.0f;

float vel_nois_X = 0.0f;
float vel_nois_Y = 0.0f;
float vel_nois_Z = 0.0f;

float gps_x = 0.0f;
float gps_y = 0.0f;

float delta_posx_test = 0.0f;
...
void navPressureAdjust(float altitude) {
    navUkfData.presAltOffset = altitude - ALTITUDE;
}
...
void AP_UKF::navUkfSetHereAsPositionTarget(void)
{
	...
	{
		navUkfSetGlobalPositionTarget(gnssData.lat, gnssData.lon);
	}
}

void AP_UKF::navUkfNormalizeVec3(float *vr, float *v)
{
    float norm;
...
void AP_UKF::navUkfNormalizeQuat(float *qr, float *q)
{
    float norm;
...
void AP_UKF::navUkfRotateVectorByQuat(float *vr, float *v, float *q)
{
    float w, x, y, z;
...
void AP_UKF::navUkfRotateVectorByRevQuat(float *vr, float *v, float *q)
{
    float qc[4];
...
void AP_UKF::navUkfRotateVecByMatrix(float *vr, float *v, float *m)
{
...
void navUkfTimeUpdate(float *in, float *noise, float *out, float *u, float dt, int n)
{
    ...
    for (i = 0; i < n; i++) 
	{
		...
		acc_nois_X = in[UKF_STATE_ACC_BIAS_X*n + i];
		acc_nois_Y = in[UKF_STATE_ACC_BIAS_Y*n + i];
		acc_nois_Z = in[UKF_STATE_ACC_BIAS_Z*n + i];
		

		// rotate acc to world frame
		navUkfRotateVecByMatrix(acc, tmp, mat3x3);
		navUkfRotateVecByMatrix(ACCP, u, mat3x3);
		acc[2] += GRAVITY;
		...
		vel_nois_X = noise[UKF_V_NOISE_VELN*n + i];
		vel_nois_Y = noise[UKF_V_NOISE_VELE*n + i];
		vel_nois_Z = noise[UKF_V_NOISE_VELD*n + i];
		...
		gyro_nois_X = in[UKF_STATE_GYO_BIAS_X*n + i];
		gyro_nois_Y = in[UKF_STATE_GYO_BIAS_Y*n + i];
		gyro_nois_Z = in[UKF_STATE_GYO_BIAS_Z*n + i];
...
//{
//  
//    y[0] = x[UKF_STATE_Q1] + noise[0]; 
//    y[1] = x[UKF_STATE_Q2] + noise[1];
//    y[2] = x[UKF_STATE_Q3] + noise[2];
//		y[3] = x[UKF_STATE_Q4] + noise[3];
//}
...
//    navUkfData.yaw = compassNormalize(navUkfData.yaw * RAD_TO_DEG);
	  heading = navUkfData.yaw * RAD_TO_DEG;
	  if(heading < 0.0f)
		heading += 360.0f;
    if(heading >= 360.0f)
		heading -= 360.0f;
	
	  navUkfData.yaw = heading;
    navUkfData.pitch *= RAD_TO_DEG;
    navUkfData.roll *= RAD_TO_DEG;

    //    x' = x cos f - y sin f
    //    y' = y cos f + x sin f
    navUkfData.yawCos = cosf(navUkfData.yaw * DEG_TO_RAD);//cosf 
    navUkfData.yawSin = sinf(navUkfData.yaw * DEG_TO_RAD);//sinf
}

static hrt_abstime preTime = 0;
hrt_abstime cruTime = 0;
hrt_abstime deltaTime = 0;
void navUkfInertialUpdate(void)
{
	...
	cruTime = ap_gyro.timestamp;
	deltaTime = cruTime-preTime;
	preTime = cruTime;
	navUkfData.ap_outerTimestep = deltaTime;
	
    srcdkfTimeUpdate(navUkfData.kf, u, ((float32_t)deltaTime)*1.0E-6 );
...
void RPY2ABCD( float roll, float pitch, float yaw, float abcd[] )
{
	static float u0,u1,u2; 
	static float cosr,sinr,cosp,sinp,cosy,siny;
	static float tmpr,tmpp,tmpy;
	static float tmpa,tmpb,tmpc,tmpd;
	tmpr = roll*DEG_TO_RAD;
	tmpp = pitch*DEG_TO_RAD;
	tmpy = yaw*DEG_TO_RAD;
	...
	cosy = cos(u2);
	siny = sin(u2);
	tmpa = cosr*cosp*cosy + sinr*sinp*siny;
	tmpb = sinr*cosp*cosy - cosr*sinp*siny;
	tmpc = cosr*sinp*cosy + sinr*cosp*siny;
	tmpd = cosr*cosp*siny - sinr*sinp*cosy;
	abcd[ 0 ] = tmpa;
	abcd[ 1 ] = tmpb;
	abcd[ 2 ] = tmpc;
	abcd[ 3 ] = tmpd; 
...
void simDoQuatUpdate(float roll, float pitch, float yaw)
{
	float noise[4];        // measurement variance
	float y[4];            // measurement(s)
	float norm;

	RPY2ABCD(roll,pitch,yaw,y);
    ...
    // normalize vector
    norm = 1.0f / sqrtf(y[0]*y[0] + y[1]*y[1] + y[2]*y[2] + y[3]*y[3]);
    y[0] = y[0] * norm;
    y[1] = y[1] * norm;
    y[2] = y[2] * norm;
	y[3] = y[3] * norm;

    srcdkfMeasurementUpdate(navUkfData.kf, 0, y, 4, 4, noise, navUkfMagUpdate);
}
...
void navUkfGpsPosUpdate(uint32_t gpsMicros, double lat, double lon, float alt, float hAcc, float vAcc)
{
    ...
    else 
	{
		...
		histIndex = (hrt_absolute_time() - (gpsMicros + navUkfData.sensorNoise[UKF_NOISE_POS_DELAY])) / navUkfData.ap_outerTimestep;
		...
		noise[0] = navUkfData.sensorNoise[UKF_NOISE_GPS_POS_N] + hAcc * sqrtf(gnssData.tDOP*gnssData.tDOP + gnssData.nDOP*gnssData.nDOP) * navUkfData.sensorNoise[UKF_NOISE_GPS_POS_M_N];
		noise[1] = navUkfData.sensorNoise[UKF_NOISE_GPS_POS_N] + hAcc * sqrtf(gnssData.tDOP*gnssData.tDOP + gnssData.eDOP*gnssData.eDOP) * navUkfData.sensorNoise[UKF_NOISE_GPS_POS_M_N];
		noise[2] = navUkfData.sensorNoise[UKF_NOISE_GPS_ALT_N] + vAcc * sqrtf(gnssData.tDOP*gnssData.tDOP + gnssData.vDOP*gnssData.vDOP) * navUkfData.sensorNoise[UKF_NOISE_GPS_ALT_M_N];
...
void navUkfGpsVelUpdate(uint32_t gpsMicros, float velN, float velE, float velD, float sAcc)
{
    ...
    histIndex = (hrt_absolute_time() - (gpsMicros + navUkfData.sensorNoise[UKF_NOISE_VEL_DELAY])) / navUkfData.ap_outerTimestep;
    histIndex = navUkfData.navHistIndex - histIndex;
    if (histIndex < 0)
	histIndex += UKF_HIST;
    if (histIndex < 0 || histIndex >= UKF_HIST)
    ...
    noise[0] = navUkfData.sensorNoise[UKF_NOISE_GPS_VEL_N] + sAcc * sqrtf(gnssData.tDOP*gnssData.tDOP + gnssData.nDOP*gnssData.nDOP) * navUkfData.sensorNoise[UKF_NOISE_GPS_VEL_M_N];
    noise[1] = navUkfData.sensorNoise[UKF_NOISE_GPS_VEL_N] + sAcc * sqrtf(gnssData.tDOP*gnssData.tDOP + gnssData.eDOP*gnssData.eDOP) * navUkfData.sensorNoise[UKF_NOISE_GPS_VEL_M_N];
    noise[2] = navUkfData.sensorNoise[UKF_NOISE_GPS_VD_N]  + sAcc * sqrtf(gnssData.tDOP*gnssData.tDOP + gnssData.vDOP*gnssData.vDOP) * navUkfData.sensorNoise[UKF_NOISE_GPS_VD_M_N];
...
//			navPressureAdjust(navUkfData.flowAlt);
    ...
    else 
	{
		...
		flowX -= (navUkfData.pitch - oldPitch) * DEG_TO_RAD * UKF_FOCAL_PX;
		flowY += (navUkfData.roll  - oldRoll)  * DEG_TO_RAD * UKF_FOCAL_PX;
    ...
    oldRoll = navUkfData.roll;
    oldPitch = navUkfData.pitch;
}
...
void navUkfZeroPos(void)
{
    ...
    y[0] = 0.0f;
    y[1] = 0.0f;
    y[2] = navUkfPresToAlt(ap_baro.pressure);
...
//    imuQuasiStatic(UKF_GYO_AVG_NUM);
    ...
    do
	{
		lastUpdate = ap_gyro.timestamp;
	//	while (lastUpdate == ap_gyro.timestamp)
	//	    yield(1);
    ...
    UKF_PRES_ALT = navUkfPresToAlt(ap_baro.pressure);
...
void navUkfInit(void)
{
	...
	ap_mag.inclination = -47.333f;
	ap_mag.declination = -4.41667f;

    // calculate mag vector based on inclination
    mag[0] = cosf(ap_mag.inclination * DEG_TO_RAD);//cosf 
    mag[1] = 0.0f;
    mag[2] = -sinf(ap_mag.inclination * DEG_TO_RAD);//sinf 

    // rotate local mag vector to align with true north
    navUkfData.v0m[0] = mag[0] * cosf(ap_mag.declination * DEG_TO_RAD) - mag[1] * sinf(ap_mag.declination  * DEG_TO_RAD);// cosf
    navUkfData.v0m[1] = mag[1] * cosf(ap_mag.declination * DEG_TO_RAD) + mag[0] * sinf(ap_mag.declination  * DEG_TO_RAD);// cosf
    ...
    navUkfData.flowRotCos = cosf(UKF_FLOW_ROT * DEG_TO_RAD);// cosf
    navUkfData.flowRotSin = sinf(UKF_FLOW_ROT * DEG_TO_RAD);// sinf
