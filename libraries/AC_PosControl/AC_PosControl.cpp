...
extern const AP_HAL::HAL& hal;
 ...
 # define POSCONTROL_POS_Z_P                    1.5f    // vertical position controller P gain default
 # define POSCONTROL_VEL_Z_P                    3.5f    // vertical velocity controller P gain default
 # define POSCONTROL_ACC_Z_P                    2.5f    // vertical acceleration controller P gain default
 ...
 # define POSCONTROL_ACC_Z_IMAX                 1000     // vertical acceleration controller IMAX gain default
 ...
 # define POSCONTROL_VEL_XY_D                   0.7f    // horizontal velocity controller D gain default
...
// their values.
//
AC_PosControl::AC_PosControl(const AP_AHRS_View& _ahrs, AC_AttitudeControl& _attitudeControl):
	ahrs(_ahrs),
...
void AC_PosControl::set_alt_target_from_climb_rate(float climb_rate_cms, float dt, bool force_descend)
{
    ...
    if ((climb_rate_cms<0 && force_descend) || (climb_rate_cms>0 && !_limit.pos_up)) {
        _pos_target.z += climb_rate_cms * dt;
    }
...
void AC_PosControl::set_alt_target_from_climb_rate_ff(float climb_rate_cms, float dt, bool force_descend)
{
    ...
    if ((_vel_desired.z<0 && (force_descend)) || (_vel_desired.z>0 && !_limit.pos_up)) 
		{
...
{
    _pos_target.z = ahrs.Pos_cm.z;
    _vel_desired.z = 0.0f;
    _flags.use_desvel_ff_z = false;
    _vel_target.z = ahrs.Vel_cm.z;
    _vel_last.z = ahrs.Vel_cm.z;
    _accel_desired.z = 0.0f;
    _accel_last_z_cms = 0.0f;
    _accel_target.z = -(ahrs.Accz_cmss + GRAVITY_MSS) * 100.0f;
    _flags.reset_accel_to_throttle = true;
	
/////$$$$$$$$$$$$  check this line before use  ????????????????????????
...
{
    return (_pos_target.z - ahrs.Pos_cm.z) ;
}
...
{
    const float curr_pos_z = ahrs.Pos_cm.z;
    float curr_vel_z = ahrs.Vel_cm.z;
...
void AC_PosControl::init_takeoff()
{
	  ...
	  const Vector3f& curr_pos = ahrs.Pos_cm;
...
//		}
    ...
    const Vector3f& curr_vel = ahrs.Vel_cm;
...
//    }
    ...
    _alt_out = (p+i+d)*0.1f;// +_motors.get_throttle_hover();

    _alt_out = constrain_float(_alt_out,-200.0f,200.0f);
...
void AC_PosControl::set_pos_target(const Vector3f& position)
{
    ...
    //_roll_target = constrain_int32(ahrs().roll_sensor,-_attitude_control.lean_angle_max(),_attitude_control.lean_angle_max());
    //_pitch_target = constrain_int32(ahrs().pitch_sensor,-_attitude_control.lean_angle_max(),_attitude_control.lean_angle_max());
}
...
{
    return get_bearing_cd(ahrs.Pos_cm, _pos_target);
}
...
void AC_PosControl::init_xy_controller()
{
    ...
    _roll_target = ahrs.roll_deg*100.0f;
    _pitch_target = ahrs.pitch_deg*100.0f;
...
{
    float ekfGndSpdLimit, ekfNavVelGainScaler = 1;
//    AP::ahrs_navekf().getEkfControlLimits(ekfGndSpdLimit, ekfNavVelGainScaler);

    Vector3f curr_pos = ahrs.Pos_cm;
	...
	else
	{
        ...
        limit_vector_length(_pos_error.x, _pos_error.y, _leash);

        _vel_target = sqrt_controller(_pos_error, kP, _accel_cms);
    }

	if( fabs(_vel_desired.x) > 25.0f )
	{
		_pos_target.x = ahrs.Pos_cm.x;
		_vel_target.x = _vel_desired.x;
	}
	
	if( fabs(_vel_desired.y) > 25.0f )
	{
		_pos_target.y = ahrs.Pos_cm.y;
	...
	{
        _vehicle_horiz_vel.x = ahrs.Vel_cm.x;
        _vehicle_horiz_vel.y = ahrs.Vel_cm.y;
    }
	
	_vel_target.x = tartget_vel_filter_x.apply(_vel_target.x);
	_vel_target.y = tartget_vel_filter_y.apply(_vel_target.y);
	
	_vehicle_horiz_vel.x = feedback_vel_filter_x.apply(_vehicle_horiz_vel.x);
	_vehicle_horiz_vel.y = feedback_vel_filter_y.apply(_vehicle_horiz_vel.y);
...
//	_accel_target.x = tartget_accel_filter_x.apply(accel_target.x);
//	_accel_target.y = tartget_accel_filter_y.apply(accel_target.y);
...
//    _limit.accel_xy = limit_vector_length(_accel_target.x, _accel_target.y, accel_max);
	...
	// limit acceleration using maximum lean angles
	float angle_max = ROLL_PITCH_ANGLE_MAX_DEGREE;
	float accel_max = POSCONTROL_ACCEL_XY_MAX;

	accel_max = MIN( accel_max, GRAVITY_MSS * 100.0f * tanf(ToRad(angle_max * 0.01f)));
	...
	_roll_target = constrain_float(_roll_target, -ROLL_PITCH_ANGLE_MAX_DEGREE, ROLL_PITCH_ANGLE_MAX_DEGREE);
	_pitch_target = constrain_float(_pitch_target, -ROLL_PITCH_ANGLE_MAX_DEGREE, ROLL_PITCH_ANGLE_MAX_DEGREE);
...
void AC_PosControl::accel_to_lean_angles(float accel_x_cmss, float accel_y_cmss, float& roll_target, float& pitch_target) const
{
    ...
    accel_forward = accel_x_cmss*ahrs.cosyaw + accel_y_cmss*ahrs.sinyaw;
    accel_right = -accel_x_cmss*ahrs.sinyaw + accel_y_cmss*ahrs.cosyaw;
...
//    roll_target = atanf(accel_right*cos_pitch_target/(GRAVITY_MSS * 100.0f))*(18000.0f/M_PI);
		roll_target = atanf(accel_right/(GRAVITY_MSS * 100.0f))*(18000.0f/M_PI);
}
...
void AC_PosControl::lean_angles_to_accel(float& accel_x_cmss, float& accel_y_cmss) const
{
    ...
    // todo: this should probably be based on the desired attitude not the current attitude
    accel_x_cmss = (GRAVITY_MSS * 100) * (-ahrs.cosyaw * sinf(ahrs.pitch_deg*DEG_TO_RAD) * cosf(ahrs.roll_deg*DEG_TO_RAD)\
										- ahrs.sinyaw * sinf(ahrs.roll_deg*DEG_TO_RAD)) / MAX(cosf(ahrs.roll_deg*DEG_TO_RAD)*cosf(ahrs.pitch_deg*DEG_TO_RAD), 0.5f);
    accel_y_cmss = (GRAVITY_MSS * 100) * (-ahrs.sinyaw * sinf(ahrs.pitch_deg*DEG_TO_RAD) * cosf(ahrs.roll_deg*DEG_TO_RAD)\
										+ ahrs.cosyaw * sinf(ahrs.roll_deg*DEG_TO_RAD)) / MAX(cosf(ahrs.roll_deg*DEG_TO_RAD)*cosf(ahrs.pitch_deg*DEG_TO_RAD), 0.5f);
}
...
//    _ekf_xy_reset_ms = ahrs().getLastPosNorthEastReset(pos_shift);
}
...
//    Vector2f pos_shift;
//    uint32_t reset_ms = ahrs().getLastPosNorthEastReset(pos_shift);
...
{
//    float alt_shift;
//    _ekf_z_reset_ms = ahrs().getLastPosDownReset(alt_shift);
}
...
//    // check for position shift
//    float alt_shift;
//    uint32_t reset_ms = ahrs().getLastPosDownReset(alt_shift);
...
// Proportional controller with piecewise sqrt sections to constrain second derivative
float AC_PosControl::sqrt_controller(float error, float p, float second_ord_lim, float dt)
{
