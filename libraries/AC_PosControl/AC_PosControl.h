...
#define ROLL_PITCH_ANGLE_MAX_DEGREE             2000.0f //
...
//    /// Constructor
    AC_PosControl(const AP_AHRS_View& _ahrs, AC_AttitudeControl& _attitudeControl);
    ...
    void set_alt_target_to_current_alt() { _pos_target.z = ahrs.Pos_cm.z ; }
	...
	float get_alt_out() const { return _alt_out; }
...
//		Vector3f get_velocity();
	...
	float get_pos_target_z(){return _pos_target.z;}
...
//protected:
    ...
    AC_AttitudeControl& attitudeControl;
	...
	Vector3f	_pos_output;			// position controller output cm/s
    Vector3f    _vel_desired;           // desired velocity in cm/s (feed forward)
    Vector3f    _vel_target;            // velocity target in cm/s calculated by pos_to_rate step
    Vector3f    _vel_error;             // error between desired and actual acceleration in cm/s
    Vector3f    _vel_last;              // previous iterations velocity in cm/s
	Vector3f	_vel_output;			// velocity controller output cm/s/s
	...
	Vector3f    _accel_output;          // acceleration controller output cm/s/s
	...
	LowPassFilter2pFloat tartget_vel_filter_x{100, 10};
	LowPassFilter2pFloat tartget_vel_filter_y{100, 10};
	
	LowPassFilter2pFloat feedback_vel_filter_x{100, 10};
	LowPassFilter2pFloat feedback_vel_filter_y{100, 10};
	...
	LowPassFilter2pFloat tartget_accel_filter_x{100, 20};
	LowPassFilter2pFloat tartget_accel_filter_y{100, 20};
